...
export async function decodeList({
  encodedList
}: {
  encodedList: string
}): Promise<BitstringStatusList> {
...
export function statusTypeMatches({
  credential
}: { credential?: any } = {}): boolean {
...
export function assertBitstringStatusListContext({
  credential
}: {
  credential?: any
} = {}): void {
 ...
 */
function _validateStatus({ credentialStatus }: { credentialStatus: any }): any {
 ...
 * @returns {undefined}
 */
function _isObject({ credential }: { credential?: any }): void {
 ...
 *   types.
 */
function _getStatuses({ credential }: { credential: any }): any[] {
  const { credentialStatus } = credential
